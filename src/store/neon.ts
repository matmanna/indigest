...
export class NeonStore implements Store {
  ...
  async upsertChannel(ch: StoreChannel): Promise<void> {
    await this.sql`INSERT INTO channels (id, name, team_id, enabled, webhook_url, auto_approve_users, approved_posters, metadata_schema, created_at)
      VALUES (${ch.id}, ${ch.name}, ${ch.teamId}, ${ch.enabled ? 1 : 0}, ${ch.webhookUrl}, ${ch.autoApproveUsers.join(",")}, ${ch.approvedPosters.join(",")}, ${ch.metadataSchema}, ${new Date().toISOString()})
      ON CONFLICT (id) DO UPDATE SET name = ${ch.name}, team_id = ${ch.teamId}, enabled = ${ch.enabled ? 1 : 0}, webhook_url = ${ch.webhookUrl}, auto_approve_users = ${ch.autoApproveUsers.join(",")}, approved_posters = ${ch.approvedPosters.join(",")}, metadata_schema = ${ch.metadataSchema}`;
  }
  ...
  async upsertMessage(msg: StoreMessage): Promise<void> {
    ...
    await this.sql`INSERT INTO messages (slack_ts, channel_id, user_id, user_name, text, timestamp, metadata)
      VALUES (${msg.slackTs}, ${msg.channelId}, ${msg.userId}, ${msg.userName}, ${msg.text}, ${msg.timestamp}, ${metadataStr}::jsonb)
      ON CONFLICT (channel_id, slack_ts) DO UPDATE SET user_id = ${msg.userId}, user_name = ${msg.userName}, text = ${msg.text}, timestamp = ${msg.timestamp}, metadata = ${metadataStr}::jsonb`;
  }
